...
interface GastroMapProps {
  points: GastroPoint[];
  activeId: number | null;
  onSelect?: (id: number) => void;
}

export function GastroMap({ points, activeId, onSelect }: GastroMapProps) {
  const gastroPoints = points;
  ...
  useEffect(() => {
    ...
    const point = gastroPoints.find((p) => p.id === activeId);
    if (!point) return;
    try {
  ...
  // Acid-yellow пин с номером для гастро-карты — отличается от красного маршрута
  const makeIconHref = (number: string, active: boolean) => {
    const fill = active ? "#a83a2c" : "#ffe600";
    const textColor = active ? "#f5f1ea" : "#0a0908";
    const svg = `<svg xmlns='http://www.w3.org/2000/svg' width='42' height='52' viewBox='0 0 42 52'>
      <path d='M21 0 C9.4 0 0 9.4 0 21 c0 15.7 21 31 21 31 s21-15.3 21-31 C42 9.4 32.6 0 21 0 z'
        fill='${fill}' stroke='#0a0908' stroke-width='2'/>
          ...
          {gastroPoints.map((p) => {
            ...
            return (
              <Placemark
                ...
                properties={{
                  hintContent: `${p.number} · ${p.name}`,
                  balloonContentHeader: `<strong style="font-family:Gotham Pro,sans-serif;text-transform:uppercase">${p.number} · ${p.name}</strong>`,
                  balloonContentBody: `<div style="font-family:Gotham Pro,sans-serif;font-size:12px;color:#3d3832">${p.address}<br/>${p.category} · ${p.avgCheck}</div>`,
                }}
              />
            );
          })}
